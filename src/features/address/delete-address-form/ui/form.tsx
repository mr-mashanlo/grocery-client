...
import { useDeleteAddressForm } from '../model/hook';
...
const DeleteAddressForm: FC<Props> = ( { id, className, ...props } ) => {
  const form = useDeleteAddressForm( id );
  ...
  return (
    <form onSubmit={handleFormSubmit} className={twMerge( 'w-full sm:w-80 relative', className )} {...props}>
      <h3 className="mb-5 text-xl text-center font-bold">Delete address</h3>
