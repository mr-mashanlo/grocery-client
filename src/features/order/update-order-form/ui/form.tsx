...
import { useUpdateOrderForm } from '../model/hook';
...
const UpdateOrderForm: FC<Props> = ( { order, className, ...props } ) => {
  const form = useUpdateOrderForm( order );
  ...
  return (
    <form onSubmit={handleFormSubmit} className={twMerge( 'w-full sm:w-80 relative', className )} {...props}>
      <h3 className="mb-5 text-xl text-center font-bold">Create order</h3>
