...
import { useDeleteOrderForm } from '../model/hook';
...
const DeleteOrderForm: FC<Props> = ( { id, className, ...props } ) => {
  const form = useDeleteOrderForm( id );
  ...
  return (
    <form onSubmit={handleFormSubmit} className={twMerge( 'w-full sm:w-80 relative', className )} {...props}>
      <h3 className="mb-5 text-xl text-center font-bold">Delete order</h3>
