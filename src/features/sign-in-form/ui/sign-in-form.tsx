...
const SignInForm: FC = () => {
  ...
  const form = useSignInForm( { onSuccess: () => navigate( '/', { replace: true } ) } );
  ...
  return (
    <form onSubmit={handleFormSubmit} className="w-full sm:w-84 relative">
      <Fieldset>
        <Legend className="mb-8 text-3xl text-center font-bold">Sign in</Legend>
        <div className="grid gap-4 sm:gap-5">
          ...
          <form.Subscribe selector={state => [ state.canSubmit, state.isSubmitting ]} children={( [ canSubmit, isSubmitting ] ) =>
            <Button disabled={!canSubmit} type="submit" className="w-full p-5 rounded-2xl bg-black text-white cursor-pointer outline-offset-3 disabled:cursor-default disabled:opacity-70">{isSubmitting ? '•••' : 'Sign in'}</Button> }
          />
        </div>
      </Fieldset>
