...
import { useDeleteCategoryForm } from '../model/hook';
...
const DeleteCategoryForm: FC<Props> = ( { id, className, ...props } ) => {
  const form = useDeleteCategoryForm( id );
  ...
  return (
    <form onSubmit={handleFormSubmit} className={twMerge( 'w-full sm:w-80 relative', className )} {...props}>
      <h3 className="mb-5 text-xl text-center font-bold">Delete category</h3>
