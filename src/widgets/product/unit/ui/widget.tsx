...
const ProductUnit: FC<Props> = ( { className, ...props } ) => {
    ...
    <section className={twMerge( 'mx-4 my-8 sm:mx-10 sm:my-20', className )} {...props}>
      <div className="grid sm:grid-cols-3 gap-4 sm:gap-5">
        <ul className="grid sm:grid-cols-2 gap-4 sm:gap-5 sm:col-span-2">
          {product.data?.images.map( image => <li key={image._id} className="block aspect-square rounded-xl overflow-hidden"><img src={image.url} alt={image.alt} className="w-full h-full" /></li> )}
        </ul>
